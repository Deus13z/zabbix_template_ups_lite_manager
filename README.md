...
Шаблон предназначен для мониторинга источников бесперебойного питания DEXP 3000VA 
(и совместимых моделей на протоколе Megatec/Ippon) через программное обеспечение 
UPS Lite Manager на Windows Server. Данные считываются напрямую из лог-файлов программы, 
что обеспечивает высокую точность и частоту обновления графиков.
...
Откройте UPS Lite Manager → Настройки → вкладка Логи и настройте следующим образом:
✅ Лог важных событий
...
Обязательно отметьте все параметры: In V, Freq, Out V, Curr, Temp, Batt V, BattLevel, Batt, Low, AVR, Err, Test, Off
Включите: Записывать только изменившийся параметр
Включите: Игнорировать небольшие изменения напряжения сети → установите значение 2.0 V
